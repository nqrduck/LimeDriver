...
std::vector<Config2HDFattr_t> getHDFAttributes(LimeConfig_t &LimeCfg) {
  std::vector<Config2HDFattr_t> HDFattr = {
      ...
      {"tgi", "TX I Gain correction", H5::PredType::NATIVE_INT,
       &LimeCfg.TX_IcorrGain, 1},
      {"tgq", "TX Q Gain correction", H5::PredType::NATIVE_INT,
       &LimeCfg.TX_QcorrGain, 1},
      {"tpc", "TX Phase correction", H5::PredType::NATIVE_INT,
       &LimeCfg.TX_IQcorrPhase, 1},
      {"rgi", "RX I Gain correction", H5::PredType::NATIVE_INT,
       &LimeCfg.RX_IcorrGain, 1},
      {"rgq", "RX Q Gain correction", H5::PredType::NATIVE_INT,
       &LimeCfg.RX_QcorrGain, 1},
      {"rpc", "RX Phase correction", H5::PredType::NATIVE_INT,
       &LimeCfg.RX_IQcorrPhase, 1},
      ...
      {"amf", "AM Frequency [Hz]", H5::PredType::IEEE_F64LE, LimeCfg.am_frq,
       (hsize_t)LimeCfg.Npulses},
      {"amp", "AM Phase [rad]", H5::PredType::IEEE_F64LE, LimeCfg.am_pha,
       (hsize_t)LimeCfg.Npulses},
      {"amd", "AM Depth", H5::PredType::IEEE_F64LE, LimeCfg.am_depth,
       (hsize_t)LimeCfg.Npulses},
      {"amm", "AM Mode", H5::PredType::NATIVE_INT, LimeCfg.am_mode,
       (hsize_t)LimeCfg.Npulses},
      {"///", "AM Frequency [1/Sa]", H5::PredType::IEEE_F64LE,
       LimeCfg.am_frq_smp, (hsize_t)LimeCfg.Npulses},
      {"fmf", "FM Frequency [Hz]", H5::PredType::IEEE_F64LE, LimeCfg.fm_frq,
       (hsize_t)LimeCfg.Npulses},
      {"fmp", "FM Phase [rad]", H5::PredType::IEEE_F64LE, LimeCfg.fm_pha,
       (hsize_t)LimeCfg.Npulses},
      {"fmw", "FM width [Hz]", H5::PredType::IEEE_F64LE, LimeCfg.fm_width,
       (hsize_t)LimeCfg.Npulses},
      {"fmm", "FM Mode", H5::PredType::NATIVE_INT, LimeCfg.fm_mode,
       (hsize_t)LimeCfg.Npulses},
      {"///", "FM Frequency [1/Sa]", H5::PredType::IEEE_F64LE,
       LimeCfg.fm_frq_smp, (hsize_t)LimeCfg.Npulses},
      ...
      {"noi", "Don't init if >0", H5::PredType::NATIVE_INT,
       &LimeCfg.override_init, 1},
...
LimeConfig_t initializeLimeConfig(int Npulses) {
  ...
  */
  ...
  LimeCfg.am_mode = new int[LimeCfg.Npulses];     // pulse AM mode (0: sinus, 1:
                                                  // triangle, 2: square)
  ...
  LimeCfg.fm_mode = new int[LimeCfg.Npulses];     // pulse FM mode (0: sinus, 1:
                                                  // triangle, 2: square)
...
int parseArguments(int argc, char **argv, LimeConfig_t &LimeCfg,
                   std::vector<Config2HDFattr_t> &HDFattrVector) {
  ...
  for (int ii_arg = 1; ii_arg < argc; ii_arg++) {
    ...
    if (argv[ii_arg][0] == '-') {
      ...
      if (curr_attr == -1 && attr2read_last == 0) {
        ...
        cout << "Missing argument: " << attr2read_last
             << " value missing for argument "
             << HDFattrVector[curr_attr_last].arg << endl;
        parse_prob = true;
      }
    ...
    if (curr_attr != -1 && attr2read != 0) {
      ...
      if (HDFattrVector[curr_attr].dType == H5::PredType::IEEE_F32LE) {
        *((float *)HDFattrVector[curr_attr].Value + attr_read) =
            atof(argv[ii_arg]);
        attr2read--;
        attr_read++;
      ...
      if (HDFattrVector[curr_attr].dType == H5::PredType::NATIVE_INT) {
        *((int *)HDFattrVector[curr_attr].Value + attr_read) =
            atoi(argv[ii_arg]);
        attr2read--;
        attr_read++;
  ...
  if (parse_prob) {
    ...
    for (int ii_attr = 0; ii_attr < no_of_attr; ii_attr++) {
      ...
      cout << "-" << HDFattrVector[ii_attr].arg << "   " << left << setw(30)
           << HDFattrVector[ii_attr].Name << ": " << HDFattrVector[ii_attr].dim
           << "x " << datatype << endl;
    }
    return 1;
  }
...
int run_experiment(LimeConfig_t LimeCfg,
                   std::vector<Config2HDFattr_t> &HDFattrVector) {
  /* Run the experiment
  ...
  */
  ...
  // initialize LimeSDR if there is a deviation in relevant parameters or if it
  // is enforced to init, not init
  // if (frqdev || sratedev || tgndev || rgndev || rlpfdev || tlpfdev || true) {
  bool override_init =
      LimeCfg.override_init > 0; // override the initialization if -noi > 0
  bool enforce_init = LimeCfg.override_init < 0; // enforce init if -noi < 0

  if (((frqdev || sratedev || tgndev || rgndev) && !override_init) ||
      enforce_init) {
    ...
    if (LMS_WriteParam(device, LMS7_GCORRI_TXTSP, LimeCfg.TX_IcorrGain) != 0)
      error();
    if (LMS_WriteParam(device, LMS7_GCORRQ_TXTSP, LimeCfg.TX_QcorrGain) != 0)
      error();
    if (LMS_WriteParam(device, LMS7_IQCORR_TXTSP, LimeCfg.TX_IQcorrPhase) != 0)
      error();
    if (LMS_WriteParam(device, LMS7_GCORRI_RXTSP, LimeCfg.RX_IcorrGain) != 0)
      error();
    if (LMS_WriteParam(device, LMS7_GCORRQ_RXTSP, LimeCfg.RX_QcorrGain) != 0)
      error();
    if (LMS_WriteParam(device, LMS7_IQCORR_RXTSP, LimeCfg.RX_IQcorrPhase) != 0)
      error();
...
*/
  ...
  for (int ii = 0; ii < chCount; ++ii) {
    ...
    rx_streams[ii].throughputVsLatency =
        1.0;                     // 1.0 max throuhput, 0.0 min latency
  ...
  if (num_phavar < 1) {
    cout << "Problem with specified number of phases (pcn) for pulses 0 to "
         << LimeCfg.Npulses - 1 << ": " << endl;
    for (int ii_pls = 0; ii_pls < LimeCfg.Npulses; ii_pls++)
      cout << setw(5) << left << LimeCfg.p_phacyc_N[ii_pls];
    cout << endl;
    cout << "Number of pulse phases must be >0 for pulses:";
    for (int ii = 0; ii < LimeCfg.Npulses; ii++)
      if (LimeCfg.p_phacyc_N[ii] < 1)
        cout << setw(2) << left << ii;
    cout << endl;
  ...
  cout << "Excitation pattern: " << exc_len << " samples (" << exc_buffers
       << " buffers with " << buffersize << " samples each)" << endl;
  ...
  for (int ii_pls = 0; ii_pls < LimeCfg.Npulses; ii_pls++) {
    ...
    fm_width_smp =
        2 * pi * LimeCfg.fm_width[ii_pls] /
        LimeCfg.srate; // width of FM in rad/sample for correct accumulation
      ...
      for (int jj = 0; jj < exc_buffers;
        ...
        for (int ii = 0; ii < buffersize;
          ...
          if ((ii + buffoffs >= pulseoffs) &
            ...
            // w = 2*pi*(ii+buffoffs-pulseoffs)*pulsefrq; // frequency*time,
            // such that each pulse begins at zero phase
            ...
            if (LimeCfg.am_frq[ii_pls] != 0) {
              w_mod = 2 * pi * (ii + buffoffs - pulseoffs) *
                      LimeCfg.am_frq_smp[ii_pls]; // inst modphase
              mlt_mod = Modfunction(w_mod + LimeCfg.am_pha[ii_pls],
                                    LimeCfg.am_mode[ii_pls]);
              mlt_mod = ((mlt_mod - 1.0) * LimeCfg.am_depth[ii_pls]) + 1.0;
              pulseamp_inst = pulseamp * mlt_mod;
            }

            // implement FM, currently by implementing the corresponding phase
            // integral analytically
            if (LimeCfg.fm_frq[ii_pls] != 0) {
              w_mod = 2 * pi * (ii + buffoffs - pulseoffs) *
                      LimeCfg.fm_frq_smp[ii_pls]; // inst modphase
              if (LimeCfg.fm_mode[ii_pls] == 0) { // cosine FM -> sine PM
                mlt_mod = Modfunction(w_mod + LimeCfg.fm_pha[ii_pls] - pi / 2,
                                      LimeCfg.fm_mode[ii_pls]);
              } else if (LimeCfg.fm_mode[ii_pls] ==
                         1) { // tirangluar FM --> quadratic PM
                mlt_mod = Modfunction(w_mod + LimeCfg.fm_pha[ii_pls] - pi / 2,
                                      LimeCfg.fm_mode[ii_pls]);

              } else if (LimeCfg.fm_mode[ii_pls] ==
                         2) { // square FM --> triangular PM
                mlt_mod =
                    Modfunction(w_mod + LimeCfg.fm_pha[ii_pls] - pi / 2, 1);
              }
              mlt_mod = LimeCfg.fm_width[ii_pls] / 2 * mlt_mod /
                        LimeCfg.fm_frq[ii_pls];
              pulsepha_inst = pulsepha + mlt_mod;

              if (LimeCfg.fm_mode[ii_pls] > 90) { // method with accumulator
                mlt_mod = Modfunction(w_mod + LimeCfg.fm_pha[ii_pls],
                                      LimeCfg.fm_mode[ii_pls] - 100);
            ...
            fsmpI = pulseamp_inst *
                    cos(w + pulsepha_inst + 2 * pi * pha_tab[ll][ii_pls]);
            fsmpQ = pulseamp_inst *
                    sin(w + pulsepha_inst + 2 * pi * pha_tab[ll][ii_pls]);
  ...
  if (tx_streams[0].dataFmt == lms_stream_t::LMS_FMT_I16) {
    ...
    for (int ii_c = 0; ii_c < 4;
      ...
      cout << "TTL-synth t" << ii_c << " toggles state after " << curr_synth[1]
           << " samples" << endl;
  ...
  if (rec_len > rep_offset) {
    cout << "Acquisition time of " << rec_len
         << " samples cannot be longer than repetition time (" << rep_offset
         << " Samples)" << endl;
    error();
  }

  cout << "Repetition and acquisition times: " << rep_offset << " Sa ("
       << rep_offset / buffersize << " buffers), " << rec_len << " Sa ("
       << rec_len / buffersize << " buffers with " << buffersize << " Sa each)"
       << endl;
  ...
  // mirror buffer: just holds a copy of the entire current record, without 32
  ...
  while (ii_acq < LimeCfg.repetitions * LimeCfg.averages * num_phavar) {
    ...
    if ((rx_metadata.timestamp >= next_RXtimestamp - samplesRead + 1) &&
      ...
      // normal case: scheduling as it should, where one reptime passed between
      // the current and the last stamp
      reps_btw_stamps = 1;

      // abnoral case: packet loss, i.e. more than one reptime passed since last
      // stamp
      if (timestampOffset < 0) {
        reps_btw_stamps =
            ceil(-(double)timestampOffset / (double)rep_offset) + 1;
        // cout << "Acq: rep " << ii_rep << ", avg " << ii_avg << ", pcyc " <<
        // ii_pcyc << " : sched/act tstamp: " << next_RXtimestamp << ", " <<
        // rx_metadata.timestamp << "   Diff to last: " << next_RXtimestamp -
        // last_RXtimestamp << " Offset: " <<  timestampOffset  << ", which
        // corresponds to " << -(double) timestampOffset /(double) rep_offset <<
        // " repetitions, currently processing " << samplesRead << " RX samples
        // with delayed fwd " << delayedAcqbufFwd << endl;
        ...
        // abort any ongoing acquisition, including delayed acquisition due to
        // offsets
        if (acquiring) {
          cout << "Packet loss during rep " << ii_rep << ", avg " << ii_avg
               << ", pcyc " << ii_pcyc << ", acq " << ii_acq << ", acqd "
               << ii_acqd << ": " << samples2Acquire
               << " Samples were not written and only the first few samples "
                  "are non-corrupted with certainty. "
               << endl;

          // substract the content of the mirror_buf from the acqbuf, just to
          // avoid that crap is being kept on the main buffer.
          acqbuf_pos = acqbuf[ii_rep * num_phavar + ii_pcyc];
          for (int ii_acqbuf = 0; ii_acqbuf < 2 * (rec_len - samples2Acquire);
               ii_acqbuf++)
            acqbuf_pos[ii_acqbuf] -= (int)mirror_buf[ii_acqbuf];
          ...
          // the acquisition counter
          //  this is actually weird here. It seems that we land here sometimes
          //  with ii_acq already advanced and sometimes with ii_acq still
          //  needing to be incremented. in fact, when resuming, we are
          //  sometimes having ii_acq one before ii_avg, or aligned, as it
          //  should so we do lose one acq and are moreover shifting the entire
          //  indexing. However, if ii_acq is uncommented, there is a segfault
          //  from advancing ii_acq once too few, so that it is better to keep
          //  this one it is probably related to delayed acqbuf fwd. the
          //  cleanest would eventually be to make a new ii_acq counter that
          //  counts the memory location and is incremented together with the
          //  ii_pcyc counters, etc. The actual ii_acq that is incremented after
          //  having acquired data, could be named ii_acqd to count for the
          //  acquired data packets.
        }
        acquiring = false; // this will be reset to true just below, which will
                           // cause those unwritten samples
        delayedAcqbufFwd = false; // ... and make sure that these unwritten
                                  // samples do not go anywhere to the buffer
      }

      // Advance counters to new position
      for (int ii = 0; ii < reps_btw_stamps; ii++) {

        // except at the very first acquisition, where the counters are already
      ...
      if (reps_btw_stamps > 1) {
        ...
        cout << "   Resuming acq at rep " << ii_rep << ", avg " << ii_avg
             << ", pcyc " << ii_pcyc << ", acq " << ii_acq << ": "
             << "Skipped " << reps_btw_stamps - 1
             << " acquisitions to restore a timestamp offset of "
             << timestampOffset << endl;
      }
      ...
      // cout << "Now at rep " << ii_rep << ", avg " << ii_avg << ", pcyc " <<
      // ii_pcyc << ", acq " << ii_acq  << ", acqd " << ii_acqd << "."<< endl;

      // Warn in case of packet loss
      // Important: We exclude here packet loss at the very beginning of the RX
      // stream. This is actually rather normal and the reason for the offset in
      // the RX buffer
      LMS_GetStreamStatus(rx_streams, &status); // Obtain RX stream stats
      if ((status.droppedPackets > 0 | status.overrun > 0 |
           status.underrun > 0) &
          ii_acq > 0) {
        cout << "Rx stream trouble! Status (overrun, underrun, dropped "
                "Packets): "
             << status.overrun << ", " << status.underrun << ", "
             << status.droppedPackets << ". Currently "
             << status.fifoFilledCount << " samples in RX stream buffer ("
             << 100.0 * ((float)status.fifoFilledCount) /
                    ((float)status.fifoSize)
             << "%)" << endl;
        // cout << "Next acq: rep " << ii_rep << ", avg " << ii_avg << ", pcyc "
        // << ii_pcyc << " : sched/act tstamp: " << next_RXtimestamp << ", " <<
        // rx_metadata.timestamp << "   Diff to last: " << next_RXtimestamp -
        // last_RXtimestamp << " Offset: " <<  timestampOffset  << " currently
        // processing " << samplesRead << " RX samples with delayed fwd " <<
        // delayedAcqbufFwd << endl;
      }
    ...
    // Do not enter into acquiring for the case that the experiment is at its
    // end
    if (ii_rep == LimeCfg.repetitions & delayedAcqbufFwd == false)
      break;
