...
*/
	...
	*/
	...
	cout << "A total of " << exc_buffers << " excitation buffers is required to fit the experiment" << endl;
	...
	if (rec_len > rep_offset) {
		cout << "Recording length of " << rec_len << " samples cannot be longer than repetition time (" << rep_offset << " Samples)" << endl;
		error();
	}	

	cout << "Rep and rectimes: " << rep_offset <<  ", " << rec_len << endl;
	cout << "Rep and rectimes in bufperiods: " << rep_offset/buffersize <<  ", " << rec_len/buffersize << endl;
...
*/
	...
	while (ii_acq < LimeCfg.repetitions * LimeCfg.averages * num_phavar) 
	{
		...
		if ((rx_metadata.timestamp >= next_RXtimestamp - samplesRead + 1) && acquire) {
			...
			// Important debug message in general, will however only be printed in case the offset gets positive (which will inevidably run into a segfault and is related to a too fast samplingrate combined with a too small rectime_secs)
			if (timestampOffset < 0) {
				cout << "Next acq: rep " << ii_rep << ", avg " << ii_avg << ", pcyc " << ii_pcyc << " : sched/act tstamp: " << next_RXtimestamp << ", " << rx_metadata.timestamp << "   Diff to last: " << next_RXtimestamp - last_RXtimestamp << " Offset: " <<  (signed long) rx_metadata.timestamp - (signed long) next_RXtimestamp  << " currently processing " << samplesRead << " RX samples with delayed fwd " << delayedAcqbufFwd << endl;
			}
